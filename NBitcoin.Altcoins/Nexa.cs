...
{
	public class Nexa : NetworkSetBase
	{
		public static Nexa Instance { get; } = new Nexa();

		public override string CryptoCode => "NEXA";
		...
		class NexaConsensusFactory : ConsensusFactory
		{
			private NexaConsensusFactory()
			{

			}
			public static NexaConsensusFactory Instance { get; } = new NexaConsensusFactory();
		...
		class NexaStringParser : NetworkStringParser
		{
			string _Prefix;
			public NexaStringParser(string prefix)
			{
				_Prefix = prefix;
			}
			...
			{
				var addr = new NexaAddr.NexaAddr.NexaAddrData()
				{
					Format = NexaAddr.NexaAddr.CashFormat.Cashaddr,
					Prefix = _Prefix,
					Hash = keyId.ToBytes(),
					Type = NexaAddr.NexaAddr.CashType.P2PKH,
					Network = network
				};
			...
			public override BitcoinScriptAddress CreateP2SH(ScriptId scriptId, Network network)
			{
				var addr = new NexaAddr.NexaAddr.NexaAddrData()
				{
					...
					Type = NexaAddr.NexaAddr.CashType.P2SH,
					Network = network
				};
				var str = NexaAddr.NexaAddr.EncodeAsCashaddr(addr);
		...
		protected override NetworkBuilder CreateMainnet()
		{
			...
			{
				SubsidyHalvingInterval = 210000 * 5,
			...
			.SetBase58Bytes(Base58Type.SECRET_KEY, new byte[] { 35 })
			.SetBase58Bytes(Base58Type.EXT_PUBLIC_KEY, new byte[] { 0x42, 0x69, 0x67, 0x20 })
			...
			.AddDNSSeeds(new[]
			{
				new DNSSeedData("nextchain.cash", "seed.nextchain.cash"),
				new DNSSeedData("nexa.org", "seeder.nexa.org"),
				new DNSSeedData("bitcoinunlimited.info", "nexa-seeder.bitcoinunlimited.info")
			})
		...
		protected override NetworkBuilder CreateTestnet()
		{
			...
			.SetBech32(Bech32Type.WITNESS_PUBKEY_ADDRESS, Encoders.Bech32("nexatest"))
			.SetBech32(Bech32Type.WITNESS_SCRIPT_ADDRESS, Encoders.Bech32("nexatest"))
			.SetMagic(0xf4f3e5f4)
			.SetPort(7230)
			.SetRPCPort(18332)
			.SetNetworkStringParser(new NexaStringParser("nexatest"))
			.SetName("nexatest")
			.AddAlias("nexa-testnet")
			.SetUriScheme("nexa")
			.AddDNSSeeds(new[]
			{
		...
		protected override NetworkBuilder CreateRegtest()
		{
			...
			.SetBase58Bytes(Base58Type.EXT_PUBLIC_KEY, new byte[] { 0x04, 0x35, 0x87, 0xCF })
			.SetBase58Bytes(Base58Type.EXT_SECRET_KEY, new byte[] { 0x04, 0x35, 0x83, 0x94 })
			.SetBech32(Bech32Type.WITNESS_PUBKEY_ADDRESS, Encoders.Bech32("nexareg"))
			.SetBech32(Bech32Type.WITNESS_SCRIPT_ADDRESS, Encoders.Bech32("nexareg"))
			.SetMagic(0xfabfb5da)
			.SetPort(18444)
			.SetRPCPort(18443)
			.SetNetworkStringParser(new NexaStringParser("nexareg"))
			.SetName("nexareg")
			.AddAlias("nexa-regtest")
			.SetUriScheme("nexa")
...
namespace NexaAddr
{
	...
	internal static class NexaAddr
	{
		...
		public class NexaAddrData
		{
			...
			public static NexaAddrData Create(CashFormat format, Network network, CashType type, byte[] hash)
			{
				return new NexaAddrData
				{
					Format = format,
					Network = network,
					Type = type,
		...
		/// <returns></returns>
		public static string EncodeAsCashaddr(NexaAddrData decoded)
		{
		...
		/// <param name="decoded"></param>
		/// <returns></returns>
		public static string EncodeAsCashaddrNoPrefix(NexaAddrData decoded)
		{
			...
			throw new Validation.ValidationError($"Invalid NexaAddrData");
		}
		...
		/// <returns></returns>
		public static NexaAddrData DecodeAddress(string address, string prefix, Network network)
		{
			try
			{
		...
		/// <returns></returns>
		private static NexaAddrData DecodeCashAddress(string address, string prefix, Network network)
		{
			//if(address.IndexOf(":") != -1)
			//{
		...
		/// <param name="address">A valid Bitcoin Cash address in any format</param>
		/// <returns></returns>
		private static NexaAddrData DecodeCashAddressWithPrefix(string address, Network network)
		{
			var decoded = CashAddr.Decode(address);
			var type = decoded.Type == "P2PKH" ? CashType.P2PKH : CashType.P2SH;
			return NexaAddrData.Create(CashFormat.Cashaddr, network, type, decoded.Hash);
		}
	}
